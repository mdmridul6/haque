...
(function ($) {
    ...
    $.fn.imageUploader = function (options) {
        ...
        let defaults = {
            ...
            preloadedRemovedInputName:'preloaded_removed',
            label: 'Drag & Drop files here or click to browse',
            extensions: ['.jpg', '.jpeg', '.png', '.gif', '.svg', '.pdf', '.PDF', '.doc', '.docx', '.DOCX'],
            mimes: ['image/jpeg', 'image/png', 'image/gif', 'image/svg+xml', 'application/pdf', 'application/vnd.openxmlformats-officedocument.wordprocessingml.document'],
            multiple:undefined,
            maxSize: undefined,
            maxFiles:undefined,
        ...
        let createContainer = function () {
            ...
            $input = $('<input>', {
                ...
                name:plugin.settings.multiple ? plugin.settings.imagesInputName + '[]' : plugin.settings.imagesInputName,
        ...
        let createImg = function (src, id, preloaded) {
            ...
            let $container = $('<div>', {class: 'uploaded-image'}),
                ...
                $button = $('<button>', {class: 'delete-image noLoadingBtn'}).attr('data-index', id).appendTo($container),
            ...
            if (preloaded) {
                ...
                let $preloaded = $('<input>', {
                    type: 'hidden',
                    name:plugin.settings.multiple ? plugin.settings.preloadedInputName + '[]' : plugin.settings.preloadedInputName,
                    value: id
                }).appendTo($container)
            ...
            $button.on("click", function (e) {
                ...
                if ($container.data('preloaded') === true) {
                    ...
                    let $preloaded = $('<input>', {
                        type: 'hidden',
                        name: plugin.settings.preloadedRemovedInputName + '[]',
                        value: id
                    }).appendTo($parent)
        ...
        let validateExtension = function (file) {
            ...
            if (plugin.settings.extensions.indexOf(file.name.replace(new RegExp('^.*\\.'), '.')) < 0) {
                plugin.settings.onError(`The file "${file.name}" does not match with the accepted file extensions: "${plugin.settings.extensions.join('", "')}"`);
        ...
        let validateMIME = function (file) {
            ...
            if (plugin.settings.mimes.indexOf(file.type) < 0) {
                plugin.settings.onError(`The file "${file.name}" does not match with the accepted mime types: "${plugin.settings.mimes.join('", "')}"`);
        ...
        let validateMaxSize = function (file) {
            ...
            if (file.size > plugin.settings.maxSize) {
                plugin.settings.onError(`The file "${file.name}" exceeds the maximum size of ${plugin.settings.maxSize / 1024 / 1024}Mb`);
        ...
        let validateMaxFiles = function (index, file) {
            ...
            if ((index + dataTransfer.items.length + plugin.settings.preloaded.length) >= plugin.settings.maxFiles) {
                plugin.settings.onError(`The file "${file.name}" could not be added because the limit of ${plugin.settings.maxFiles} files was reached`);
        ...
        let setPreview = function ($container, files) {
            ...
            $(files).each(function (i, file) {
                ...
                if (file.type.startsWith('image/')) {
                    ...
                    // If it's not an image, set a default image
                    $uploadedContainer.append(createImg(defaultPdfPath, dataTransfer.items.length - 1));
                }else {
                    // If it's not an image, set a default image
                    $uploadedContainer.append(createImg(defaultDocPath, dataTransfer.items.length - 1));
                }
